...
def run_youtube_agent(user_id: str) -> dict:
    ...
    provider  = get_provider(os.getenv("LLM_PROVIDER", "claude"), use_sonnet=False)
    ...
    for channel in channels:
        ...
        for video in videos:
            ...
            try:
                relevance = json.loads(rel_response.content)
            ...
            deep_provider = get_provider(os.getenv("LLM_PROVIDER", "claude"), use_sonnet=True)
            ...
            try:
                insights = json.loads(ins_response.content)
            except json.JSONDecodeError:
                continue
